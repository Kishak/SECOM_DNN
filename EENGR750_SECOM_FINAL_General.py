...
print("Statistical Analysis Input Dataframe",X_df.describe(include='all'))
print("Statistical Analysis Output Dataframe",Y_df.describe(include='all'))
#print(Z_df.describe(include='all')) # same as Y_Path
print("Input Data Shape:",X_df.shape,"Output Data Shape:", Y_df.shape)
# Take a sample of five rows of each vector group
print('Sample Input dataframe: ',X_df.sample(5))
print('Sample Output dataframe: ',Y_df.sample(5))
# print(Y_df.head())
# print(Y_df.tail())
# print(Z_df.head())
# print(Z_df.tail())
...
# Removing columns where >300 values are missing for a sensor feature column i.e., 20% of data per attribute
print('Dropping columns with >20% missing values: ', X_df.columns.where(X_df.isna().sum()>300).dropna().values)
X_df = X_df.drop(columns = X_df.columns.where(X_df.isna().sum()>300).dropna())
...
print("Train and Test Shape For PCA Selected:", X_train.shape, X_test.shape)
...
print("Confusion Matrix for Training Set:",CM_LR_tr)

LR_pred_te = LR_clf.predict(X_test)
CM_LR_te = confusion_matrix(y_test, LR_pred_te)
print("Confusion Matrix for Testing Set:",CM_LR_te)
...
print('DNN Accuracy: %.2f' % (accuracy*100))
